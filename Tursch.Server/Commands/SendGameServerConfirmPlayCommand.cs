...
{
    internal class SendGameServerConfirmPlayCommand : ICommand
    {
        private readonly ServerViewModel _viewModel;
        private readonly SignalRServerService _serverService;

        public SendGameServerConfirmPlayCommand(ServerViewModel viewModel, SignalRServerService serverService)
        {
        ...
        public async void Execute(object? parameter)
        {
            try
            {
                List<string> cards = (List<string>)parameter;

                await _serverService.GameServerSendPlayConfirmation(cards);
